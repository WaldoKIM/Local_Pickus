			...
			<div class="view">
				...
				<div class="mob">
					...
					<div class="customer" id="mobileInfo2">
						...
						echo "<dd class='col-xs-11'>" . $master['elevator_yn'] . " / " . $master['floor'] . "층" . "</dd>";
						?>
					</div>
					...
					<tr>
						<td class="info" id="mainTitle">
							...
							<dl>
								...
								<dd class="col-xs-9"><?php echo $master['elevator_yn']; ?>/<?php echo $master['floor']; ?>층</dd>
								<dt class="col-xs-3">예산</dt>
								<dd class="col-xs-9"><?php echo number_format($master['price']); ?>원</dd>
								<dt class='col-xs-3'>견적마감일</dt>
								<dd class='col-xs-9'><?php
								...
								<dt class='col-xs-3'>배송요청일</dt>
								<dd class='col-xs-9'><?php echo $master['date_req']; ?></dd>
							</dl>
							<?php
							echo "<ul class='row'>";
							echo "<li class='col-xs-6'>";
				...
				<table style="margin-top: 0; padding-top: 0;" class="requst_list" id="subDetail">
					...
					for ($i = 0; $row = sql_fetch_array($info); $i++) {
						echo "<tr>";
						echo "<td>" . $row['cate'] . "</td>";
						echo "<td>" . $row['item_cat_dtl_s'] . $row['cate_name'] . "</td>";
						echo "<td>" . $row['qty'] . "</td>";
						echo "</tr>";
					}
					?>
...
<!-- 모달 수정 CSS 추가 with KJS dance -->
<link rel="stylesheet" type="text/css" href="/css/modalx.css" />

<div style="z-index: 99999999;" class="modal fade modal_table" id="modal_price_detail" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			<div style="border-bottom:none; margin:0; padding:0; padding-top:10px;" class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h4 style="color:#1379cd; font-weight:800; text-align:center;" class="modal-title">상세견적서</h4>
			...
			<div class="modal-body" style="padding-top:0;">
				<form name="frmpricedetail" action="<?php echo G5_URL; ?>/estimate/estimate_form_match_update.php" method="post" enctype="multipart/form-data" autocomplete="off">
					...
					<div class="form-group" style="border-bottom:none;margin-bottom:5px; padding-bottom:0;">
						...
						<h4 style="color:#1379cd; font-weight:800; text-align:center;" class="modal-title">상품 사진</h4>
						</div>

					</div>

					<div class="form-group" style="border-bottom:none; margin-bottom:5px; padding-bottom:5px;">
						<!-- <div class="row">
							<span class="tit_modal_match">물품내역(우측 아이콘 클릭)</span>
						...
						<div class="row" id="list_article" style="display:flex; flex-direction:column;">

							<div class="row" id="multiList">
							

								<div class='form_new add_pro formflex'>
									<div class='add_name col-xs-5 w20 m30'>
										<p class="tit_modal_match subtitle msubtitle">상품명</p>
									</div>
									
									<input placeholder='상품 이름을 입력해 주세요' type='text' name='pro_name[]' class="pro_name w70">
									<div class='add_name col-xs-5 w10'></div>
									
									<div class='add_name col-xs-5 w25 m40'>
										<p class="tit_modal_match subtitle msubtitle">상품 가격</p>
									</div>
									<!--클릭시 0을 지우지 않고 바로 가격 입력 가능하게 수정 with KJS dance 20220118-->								
									<input placeholder='가격' type="number"  onfocus="this.select()" name='pro_price[]' class="pro_price w70" id="pro_price1" >										
										<p>원</p>
										<div class='add_name col-xs-5 w20'>
											<a class="add_line addbtn" id="add_goods" href="#">추가</a>
										</div>		
								</div>											
					...
					<hr class="border">
					<div class="form-group one_line noborder">
						...
						<ul class="row">
							...
							<li class="col-xs-6">
								<label style="width: 100%;" class="box">
									<input type="radio" id="shipping_off" name="shipping_check" value="2" checked=""><i>
										<p>없음</p>
									</i></label>
							</li>
						</ul>
					...
					<div class="form-group" style="border:0 !important; margin-bottom:0; padding-bottom:0;">
						<input style="display: none;" type="number" onfocus="this.select()" value="0" name="shipping_pro" id="del_fee">
					</div>
					<div class="form-group one_line noborder" >
						<p  class="tit_modal_match msmallfont">AS 보증/교환</p>
						<ul class="row">
							<li class="col-xs-6">
								<label style="width: 100%;" class="box">
									<input type="radio" name="as_pro" id="as_on" value="1" checked=""><i>
										<p>가능</p>
									</i></label>
							</li>
							<li class="col-xs-6">
								<label style="width: 100%;" class="box">
									<input type="radio" name="as_pro" id="as_off" value="2"><i>
										<p>불가능</p>
									</i></label>
							</li>
						</ul>
					</div>
					<div class="form-group" style="border:0 !important; margin-bottom: 0; padding: 0;">
					...
					<div class="form-group" style="border:0 !important; margin-bottom:5px; padding-bottom:0;">
						<p class="tit_modal_match subtitlebold">참고사항</p>
						<textarea class="addcomment" name="match_desc"></textarea>
					</div>
					<hr class="border">
					<div class="form-group" style="border:0 !important; padding-bottom:0; margin-bottom:0;">
						<ul class="row">
							<li class="col-xs-12">
								<p class="tit_modal_match" style="margin-bottom:0; display:inline; margin-right:2vw; font-weight: 700;">총 금액</p>
								<input type="text" name="total_price" id="total_price" value="" readonly="" style="width:75%; border-radius:5px;  " />
							</li>
						</ul>
					</div>
					<div class="btn_wrap mt5">
						<ul class="row">
...
<script type="text/javascript">
	jQuery(document).ready(function() {
		...
		$('#btn_article').click(function() {
			if (btn_article == 0) {
				btn_article = 1;
				$('#list_article').slideDown();
			} else {
				btn_article = 0;
				$('#list_article').slideUp();
			}
		});
	...
	$("#add_goods").click(function() {
		//with KJS dance 20220118 최대 등록 상품수 제한 *AAA
	if(estimateCnt>=5){
		alert('최대 6개 상품까지 등록 가능합니다'); 	
    return false
    }
		...
		vHtml += "<div style='display: flex; flex-direction: row;  flex-wrap: nowrap; align-items: center;' class='form_new add_pro'>";
		vHtml += "<div class='add_name col-xs-5 w20'>";
		vHtml += "<p class='tit_modal_match subtitle'>상품명</p>";
		vHtml += "</div>";
		vHtml += "<input placeholder='상품 이름을 입력해 주세요' type='text' name='pro_name[]' class='pro_name w70'>";
		vHtml += "<div class='add_name col-xs-5 w10'></div>";
		vHtml += "<div class='add_name col-xs-5 w20'>";
		vHtml += "<p class='tit_modal_match subtitle'> 상품 가격</p>";
		vHtml += "</div>";
		vHtml += "<input placeholder='가격' type='number'  onfocus='this.select()' name='pro_price[]' class='pro_price w70' id='pro_price' "+ i +"'>";
		vHtml += "<p>원</p>";
		vHtml += "<div class='add_name col-xs-5 w20'>";				
		vHtml += "<a class='delete_item add_line delbtn' href='javascript:' >삭제</a>";
		vHtml += "</div>";
