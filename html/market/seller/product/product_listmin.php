...
// 상품정보변경
if ($_GET[hidden_goods_idx]) {
	$db->update("cs_goods", "display='$_GET[display]', main_position='$_GET[main_position]', sub_position='$_GET[sub_position]' where idx='$_GET[hidden_goods_idx]'");
}
...
if ($_GET[listNo]) {
	...
	$listNo = $BOARD_DATA[listNo];
}
if ($_GET[startPage]) {
	$startPage = $_GET[startPage];
} else {
	$startPage	= $BOARD_DATA[startPage];
}
if ($_GET[totalList]) {
	$totalList = $_GET[totalList];
} else {
	$totalList	= $BOARD_DATA[totalList];
}
...
if ($_GET[search_order]) {
	...
	$search_order	= urldecode($SEARCH_ITEM[search_order]);
}
if ($_GET[part_code1]) {
	$part_code1 = $_GET[part_code1];
} else {
	$part_code1 = $SEARCH_ITEM[part_code1];
}
if ($_GET[part_code2]) {
	$part_code2 = $_GET[part_code2];
} else {
	$part_code2 = $SEARCH_ITEM[part_code2];
}
if ($_GET[part_code3]) {
	$part_code3 = $_GET[part_code3];
} else {
	$part_code3 = $SEARCH_ITEM[part_code3];
}
if ($_GET[search_name]) {
	$search_name = $_GET[search_name];
} else {
	$search_name = $SEARCH_ITEM[search_name];
}
if ($_GET[search_media]) {
	$search_media = $_GET[search_media];
} else {
	$search_media = $SEARCH_ITEM[search_media];
}
if ($_GET[search_code]) {
	$search_code = $_GET[search_code];
} else {
	$search_code = $SEARCH_ITEM[search_code];
}
if ($_GET[search_content]) {
	$search_content = $_GET[search_content];
} else {
	$search_content = $SEARCH_ITEM[search_content];
}
if ($_GET[search_keyword]) {
	$search_keyword = $_GET[search_keyword];
} else {
	$search_keyword = $SEARCH_ITEM[search_keyword];
}
if ($_GET[addsearch]) {
	$addsearch = $_GET[addsearch];
} else {
	$addsearch = $SEARCH_ITEM[addsearch];
}
if ($_GET[position_main]) {
	$position_main = $_GET[position_main];
} else {
	$position_main = $SEARCH_ITEM[position_main];
}
if ($_GET[position_sub]) {
	$position_sub = $_GET[position_sub];
} else {
	$position_sub = $SEARCH_ITEM[position_sub];
}
...
<script language="javascript">
	...
	function goodsEdit(mv_data) {
		...
		if (choice) {
			location.href = 'product_edit.php?search_items=<?= $SEARCH_DATA ?>&board_data=' + mv_data;
		} else {
			return;
		}
	}
	...
	function goodsDel(mv_data) {
		...
		if (choice) {
			location.href = 'product_del_ok.php?search_items=<?= $SEARCH_DATA ?>&board_data=' + mv_data;
		} else {
			return;
		}
	}
...
<div id="load">
	<div class="sns_content">
		...
		<div class="sns-go">
			<img class="sns_img" src="../img/product_guide.png" alt="">
		</div>
	</div>
</div>
						...
						<tr>
										...
										<td height="150" align="center" valign="top" bgcolor="#FFFFFF" class="menu">
																							...
																							<tr>
																								<td bgcolor="#E9F2F8">
																									<table width="100%">
																										<tr>
																											<td>
																												카테고리 검색이후 해당 카테고리 내에서 키워드 검색을 이용을 하시려면 <br>
																												<font color="red">카테고리내 검색</font>을 선택하신 검색을 이용하시기 바랍니다.<br>
																												카테고리내 검색을 이용하지 않을 경우에는 전체 상품를 기준으로 검색됩니다.<br>
																												<font color="red">중지상품 검색</font>은 중지된 상품내에서만 검색됩니다.<br>
																												1. 키워드로 물품을 검색하여 업체에서 등록한 물품을 관리 할 수 있습니다.<br>
																												2. 카테고리별로 물품을 검색하여 업체에서 등록한 물품을 관리 할 수 있습니다.<br>
																											</td>
																										</tr>
																									</table>
																								</td>
											...
											<table width="100%" height="25" class="menu" style='border-collapse: collapse'>
												<form action="<?= $_SERVER[PHP_SELF]; ?>" method="get" name="goods_form">
													...
													<tr>
														<td>
															<div class="product_list_search_flex" id="customertable_divcont">
																	...
																	<div class="customertable_divcenter_1">
																		<table width="100%" class="">
																				...
																				<tr style="display:none;">
																					<td height="35" bgcolor="#178cdb" class='td_title_font'>키워드 검색</td>
																				</tr>
																				<tr>
																				
																				<td class="product_keyword_search_flex" style='padding:5px;'>
																					
																					<table style="width:100%;">
																						<? if ($part_code1 || $part_code2 || $part_code3) { ?>
																							<tr>
																								<td style="font-size: 14px !important; font-weight: bold !important;" class='sensbody'>
																									<?  //카테고리 명을 표기 위한 쿼리들
																									if ($part_code1) {
																										$part_code1_row = $db->object("cs_part", "where part1_code='$part_code1' and part_index=1");
																										$part_name .= "1차 카테고리 : " . $part_code1_row->part_name . "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
																									}
																									if ($part_code2) {
																										$part_code2_row = $db->object("cs_part", "where part2_code='$part_code2' and part_index=2");
																										$part_name .= "2차 카테고리 : " . $part_code2_row->part_name . "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
																									}
																									if ($part_code3) {
																										$part_code3_row = $db->object("cs_part", "where part3_code='$part_code3' and part_index=3");
																										$part_name .= "3차 카테고리 : " . $part_code3_row->part_name . "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
																									}
																									?>
																									<?= $part_name; ?>
																								</td>
																						...
																						<tr class="product_list_tr_flex">
																							<td style="display:none;" class='product_list_keyword_btn keyword_flex contenL sensbody' height='40'>
																								<input style="display:none;" class="src_radio" type="checkbox" value="1" id="addsearch" name="addsearch" <? if ($addsearch == 1) { ?>checked<? } ?> onclick="part_value_auto()"><label class="product_list_src_btn" for="addsearch">카테고리내 검색 </label> 
																								<input style="display:none;" class="src_radio" type="checkbox" value="1" id="search_name" name="search_name" <? if ($search_name == 1) { ?>checked<? } ?> onclick="check_item_number()"><label class="product_list_src_btn" for="search_name">중지상품 검색</label> 
																							</td>
																							<td class="product_list_stop_btn keyword_flex"> 
																								<select style="display:none;" class="product_list_select_all product_font input_design itemSelect category_select" name="search_item">
																								...
																								<div class="search_form_search">
																									<input type='text' name='search_keyword' value='<?= $search_keyword ?>' class="search_input">
																									<a class="search_btn" href='javascript:' onClick="product_search_go()"><img src="../img/search.png" alt=""></a>
																								</div>	
																							</td>
																						</tr>
																						<tr style="display:none;">
																							<td class='sensbody'>
																								<input type="text" name="search_code" class="formText" value='<?= $search_code ?>' onclick="document.goods_form.search_name.checked=false">
																								<font color='000000'>수량검색</font>-입력한 수량과 같거나 이하일경우 검색되며 <font color="red">중지상품 및 무제한설정은 제외</font>됩니다.
																							</td>
																						</tr>
																...
																<div id="customertable_divLeft_view">
																	<div class="customertable_divLeft">
																		<table width="100%" class="">
																			<tr style="display:none;">
																				<td height="35" bgcolor="#178cdb" class='td_title_font'>카테고리별 검색</td>
																			</tr>
																			<tr>
																				<td style='padding:5px;'>																		
																					<div style="margin-bottom: 2%; justify-content: center;" class='oolimbox-wrapper oolimbox-grid3 category_flex'>
																						<div style="display:none;" class='oolimbox-col_3dan'>
																							<select class="product_list_category_select product_font input_design itemSelect category_select" name="select1" size="1" class='itemSelect' style="width:98%;" onChange='change(1, this.form.select1.selectedIndex, this.form)'>
																						...
																						<div style="display:none;" class='oolimbox-col_3dan'>
																							<select class="product_list_category_select product_font input_design itemSelect category_select" name="select2" size="1" class='itemSelect' style="width:98%;" onChange='change(2, this.form.select2.selectedIndex, this.form)'>
																						...
																						<div style="display:none;" class='oolimbox-col_3dan' style='text-align:center;'>
																							...
																							<select class="product_font input_design itemSelect category_select" name="select3" size="1" class='itemSelect' style="width:98%;" onChange='change(3, this.form.select3.selectedIndex, this.form)'>
																							</select>
																						</div>
																					</div>
																				</td>
											...
											while ($row = mysqli_fetch_object($result)) {
												...
												$new_img = "";
												$hit_img = "";
												if ($admin_stat->new_mark && $row->new_mark) {
													$new_img = $page->newImg($row->register, $admin_stat->new_mark, $row->new_mark);
												}
												if ($new_img) $new_img = str_replace("./images/", "../images/", $new_img);
												// hit IMG
												if ($admin_stat->hit_mark && $row->hit_mark) {
													$hit_img = $page->hitImg($admin_stat->hit_mark, $row->click, $row->hit_mark);
												}
							...
							<? if ($newPart == 1) { ?>
								<table id="list_flex" width="100%" class="table_all">
									<tr class="list_flex_title" bgcolor="#ececec">
										<td class="product_list_category_font">
											<?
													$part_name = "";
													if ($row->part1_code) {
														$part_code1_row = $db->object("cs_part", "where part1_code='$row->part1_code' and part_index=1");
														$part_name .= "1차 카테고리 <font color='#1379cd'>" . $part_code1_row->part_name . "</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
													}
													if ($row->part2_code) {
														$part_code2_row = $db->object("cs_part", "where part2_code='$row->part2_code' and part_index=2");
														$part_name .= "2차 카테고리 <font color='#1379cd'>" . $part_code2_row->part_name . "</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
													}
													if ($row->part3_code) {
														$part_code3_row = $db->object("cs_part", "where part3_code='$row->part3_code' and part_index=3");
														$part_name .= "3차 카테고리 <font color='#1379cd'>" . $part_code3_row->part_name . "</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
													}
											?>
											<?= $part_name; ?>
										</td>
									...
									<tr class="order_border">
										<td class='order_form'>
											<div class="product_list_form">
												<img src="../../data/goodsImages/<?= $row->images1 ?>" border="0" class='resize_itemS' id='img<?= $row->idx; ?>'>
												<?/*
														<img src="../thumbnail.img.php?ThumbEncode=<?=$ThumbEncode?>" border="0" class='resize_itemS' id='img<?=$row->idx;?>' style='margin:5px auto;'><br>
														*/ ?>
												<span style="font-size:12pt; display:none;" class='noneoolimmoM_on'><?= $db->stripSlash($row->name); ?></span>
												<input style="display:none;" id="img_file" type="file" size='1' name="itemimg" onChange="javascript:itemimgChange(document.form_<?= $form_name ?>);" class='file_text'>
												<label style="display:none;" class="src_label" for="img_file">사진첨부</label>
												<div class="product_list_content">
													<p class="trade_font_bold"><?= $db->stripSlash($row->name); ?></p>
													<p class="trade_font_bold"><?= number_format($row->shop_price); ?>원</p>
														<select name="display1" class="input input_design category_select" onChange="javascript:goodsChange(document.form_<?= $form_name ?>);">
											<? if($row->number > 0){ ?>
												<option value="1" <? if ($row->display == 1) {
																		echo ("selected");
																	} ?>>판매중</option>
												<option value="0" <? if ($row->display == 0) {
																		echo ("selected");
																	} ?>>숨기기</option>
											</select>
											...
											<div class="product_list_btn_form">
												<a href="javascript:goodsEdit('<?= $board_data; ?>');" class='btn_guide1'>수정</a>
												<a href="javascript:goodsDel('<?= $board_data; ?>');" class='btn_guide1'>삭제</a>
												<a href="https://repickus.com/market/main/product_view.php?part_idx=<?=$row->part_idx;?>&goods_data=<?=$goods_data;?>" class='btn_guide1'>상세보기</a>					
											</div>
										</td>
																
										

										<td style="display:none; text-align: center; font-size:12pt;" width="20%" height="25" class='border_none tabletd_all tabletd_small noneoolimmoL'>
											&nbsp;<span class='noneoolim'><img src="../img/product_list_2013icon2.gif" alt="재고" align="absmiddle" border="0"><? if ($row->unlimit == 1) { ?><font color='4876E5'>무제한</font><? } else { ?><font color='4876E5'><?= $row->number ?><? } ?></font>&nbsp;<?= $hit_img; ?>&nbsp;<?= $new_img; ?><br>&nbsp;<font style="display:none;" color="A7A7A7">제조회사 : <?= $row->company ?></font></span>
										...
										<td style="display:none" width="15%" height="25" class='border_none sensO mmenu1 tabletd_all noneoolim'>
											<? if ($row->subst == 1) { ?>
											...
											<? } ?>
											<font color='8BA5E6'><?= number_format($row->old_price); ?>&nbsp;원</font><br>
											<font color='E44141'><?= number_format($row->shop_price); ?>&nbsp;원</font>
										</td>
